...
export interface BlockEvent { block: number; ts: number; mid: number; bestBid: number; bestAsk: number; spreadBps: number; decision: Decision | null; quote: Quote | null; fill: Fill | null; resting: { bidMon: number; askMon: number }; risk?: {paperOnly: boolean; reason: string | null; halted: boolean; modelCalls: number}; position: Position; totals: Totals }
